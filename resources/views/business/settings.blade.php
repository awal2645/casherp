...
@section('css')
    <link rel="stylesheet" href="{{ asset('css/smtp-settings.css?v='.$asset_v) }}">
@stop
...
@php
    $activeBusiness = auth()->user()->accessibleBusinesses()->whereKey(session('business.id'))->first();
    $mayManageClosure = $activeBusiness
        && ((int) $activeBusiness->owner_id === (int) auth()->id()
    ...
    <div class="row tw-mt-6">
        <div class="col-sm-12">
            <div class="box box-default">
                <div class="box-body tw-flex tw-flex-col sm:tw-flex-row sm:tw-items-center sm:tw-justify-between tw-gap-3">
                    <div>
                        <strong>Company account and data</strong>
                        <p class="text-muted tw-mb-0">Review the secure recovery-window process for closing only this company.</p>
                    </div>
                    <a class="btn btn-default" href="{{ route('business.account-closure.show') }}">Manage company closure</a>
                </div>
            </div>
        </div>
    </div>
...
<script src="{{ asset('js/smtp-settings-react.js?v='.$asset_v) }}" defer></script>
@endsection
