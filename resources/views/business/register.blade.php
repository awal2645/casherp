...
@section('content')
    ...
    <div class="col-md-8 col-xs-12 col-md-offset-2 tw-mt-6 casherp-onboarding-shell">
        @if(!empty($selected_package))
            <div class="alert alert-info tw-rounded-xl" role="status">
                <strong>{{ $selected_package->name }}</strong>
                &mdash;
                {{ (int) $selected_package->max_businesses === 0 ? 'Unlimited' : $selected_package->max_businesses }} companies,
                {{ (int) $selected_package->location_count === 0 ? 'unlimited' : $selected_package->location_count }} branches per company.
                <a href="{{ route('pricing') }}" class="pull-right">Change package</a>
            </div>
        @endif
        <div
            ...
            {!! Form::open([
                ...
                'data-draft-key' => 'casherp:first-company-registration',
            ]) !!}
            <div class="casherp-onboarding-steps" aria-label="Registration stages">
                <div class="casherp-onboarding-step" data-registration-step="company"><span class="casherp-onboarding-step-number">1</span><div><strong>Company</strong><span>Identity and location</span></div></div>
                ...
                <div class="casherp-onboarding-step" data-registration-step="owner"><span class="casherp-onboarding-step-number">3</span><div><strong>Owner access</strong><span>Secure administrator login</span></div></div>
            </div>
...
@section('javascript')
    <script>
        ...
        window.CASHERP_CHECK_USERNAME_URL = @json(route('business.postCheckUsername'));
        window.CASHERP_CHECK_EMAIL_URL = @json(route('business.postCheckEmail'));
    </script>
