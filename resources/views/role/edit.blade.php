...
<section class="content">
    ...
    @component('components.widget', ['class' => 'box-primary'])
          ...
          <div class="row check_group">
            ...
            <div class="col-md-9">
              ...
              <div class="col-md-12">
                <div class="checkbox">
                  <label>
                    {!! Form::checkbox('permissions[]', 'purchase.document.share', in_array('purchase.document.share', $role_permissions),
                    [ 'class' => 'input-icheck']); !!} Share approved purchase documents
                  </label>
                </div>
              </div>
        ...
        @if(!empty($common_settings['enable_purchase_requisition']) || !empty($common_settings['enable_purchase_order']))
          <div class="row check_group">
            <div class="col-md-1">
              <h4>@lang('lang_v1.procurement_workflow')</h4>
            </div>
            <div class="col-md-2">
              <div class="checkbox">
            ...
            <div class="col-md-9">
              @foreach([
                'procurement.submit' => 'procurement_submit',
                'procurement.quote.manage' => 'procurement_manage_quotes',
                'procurement.approve.manager' => 'procurement_manager_approval',
                'procurement.approve.finance' => 'procurement_finance_approval',
                'procurement.approve.admin' => 'procurement_admin_approval',
                'procurement.audit.view' => 'procurement_view_audit'
              ] as $procurement_permission => $procurement_label)
                <div class="col-md-12">
                  <div class="checkbox">
                    <label>
                      {!! Form::checkbox('permissions[]', $procurement_permission, in_array($procurement_permission, $role_permissions), ['class' => 'input-icheck']); !!}
                      {{ __('lang_v1.'.$procurement_label) }}
                    </label>
                  </div>
                </div>
              @endforeach
        ...
        <div class="row check_group">
          <div class="col-md-1"><h4>Subscription & account</h4></div>
          <div class="col-md-2"><div class="checkbox"><label><input type="checkbox" class="check_all input-icheck"> {{ __('role.select_all') }}</label></div></div>
          <div class="col-md-9">
            <div class="col-md-12"><div class="checkbox"><label>{!! Form::checkbox('permissions[]', 'premium_modules.purchase', in_array('premium_modules.purchase', $role_permissions), ['class' => 'input-icheck']); !!} Request premium modules and additional capacity</label></div></div>
            <div class="col-md-12"><div class="checkbox"><label>{!! Form::checkbox('permissions[]', 'business.account_closure.manage', in_array('business.account_closure.manage', $role_permissions), ['class' => 'input-icheck']); !!} Schedule or cancel company closure <small class="text-danger">(sensitive)</small></label></div></div>
          </div>
        </div>
        <hr>

        <div class="row check_group">
          <div class="col-md-1"><h4>Data imports</h4></div>
          <div class="col-md-2"><div class="checkbox"><label><input type="checkbox" class="check_all input-icheck"> {{ __('role.select_all') }}</label></div></div>
          <div class="col-md-9">
            @foreach([
              'data_import.view' => 'View permitted import history and row errors',
              'data_import.manage' => 'Upload, validate and cancel imports',
              'data_import.approve' => 'Approve staged rows for live processing',
              'data_import.rollback' => 'Roll back imports within the package window (sensitive)'
            ] as $import_permission => $import_label)
              <div class="col-md-12"><div class="checkbox"><label>{!! Form::checkbox('permissions[]', $import_permission, in_array($import_permission, $role_permissions), ['class' => 'input-icheck']); !!} {{ $import_label }}</label></div></div>
            @endforeach
          </div>
        </div>
        <hr>

        @if(app(\App\Services\FeatureAccessService::class)->enabled('property_management'))
          <div class="row check_group">
            <div class="col-md-1"><h4>Property management</h4></div>
            <div class="col-md-2">
              <div class="checkbox"><label><input type="checkbox" class="check_all input-icheck"> {{ __('role.select_all') }}</label></div>
            </div>
            <div class="col-md-9">
              @foreach([
                'property.view' => 'View dashboards, records and reports',
                'property.manage' => 'Manage properties, units, tenants and leases',
                'property.rent.manage' => 'Generate rent and record tenant payments',
                'property.maintenance.manage' => 'Manage and complete maintenance work',
                'property.viewings.manage' => 'Manage viewing requests, approvals and schedule',
                'property.accounting.manage' => 'Configure and reverse property accounting postings',
                'property.access.manage' => 'Assign users and roles to specific properties (sensitive)'
              ] as $property_permission => $property_label)
                <div class="col-md-12">
                  <div class="checkbox"><label>{!! Form::checkbox('permissions[]', $property_permission, in_array($property_permission, $role_permissions), ['class' => 'input-icheck']); !!} {{ $property_label }}</label></div>
                </div>
              @endforeach
            </div>
          </div>
        ...
        @if(app(\App\Services\FeatureAccessService::class)->enabled('smart_documents'))
          <div class="row check_group">
            <div class="col-md-1"><h4>Smart documents</h4></div>
            <div class="col-md-2"><div class="checkbox"><label><input type="checkbox" class="check_all input-icheck"> {{ __('role.select_all') }}</label></div></div>
            <div class="col-md-9">
              @foreach([
                'smart_documents.view' => 'View company documents',
                'smart_documents.create' => 'Create document drafts',
                'smart_documents.update' => 'Edit document drafts',
                'smart_documents.issue' => 'Issue and advance documents',
                'smart_documents.void' => 'Void documents',
                'smart_documents.share' => 'Create and revoke secure links',
                'smart_documents.payment.create' => 'Record payments against issued documents',
                'smart_documents.payment.reverse' => 'Reverse payment entries with a reason',
                'smart_documents.settings' => 'Manage names, numbering and approved terms'
              ] as $document_permission => $document_label)
                <div class="col-md-12"><div class="checkbox"><label>{!! Form::checkbox('permissions[]', $document_permission, in_array($document_permission, $role_permissions), ['class' => 'input-icheck']); !!} {{ $document_label }}</label></div></div>
              @endforeach
              @if(!empty($document_permission_types) && $document_permission_types->isNotEmpty())
                <div class="col-md-12"><hr class="tw-my-2"><strong>Allowed document types</strong><br><small class="text-muted">Use this for Finance, Front Desk, Property, Events, Procurement, or other department-style roles. Company settings can replace this default for a type with an explicit list of named users, roles, or HR departments.</small></div>
                @foreach($document_permission_types->groupBy('category') as $document_category => $permission_types)
                  <div class="col-md-12 tw-mt-2"><strong>{{ ucfirst($document_category) }}</strong></div>
                  @foreach($permission_types as $permission_type)
                    @php($type_permission = 'smart_documents.type.'.$permission_type->code)
                    <div class="col-md-6"><div class="checkbox"><label>{!! Form::checkbox('permissions[]', $type_permission, in_array($type_permission, $role_permissions), ['class' => 'input-icheck']); !!} {{ $permission_type->pivot->display_name ?: $permission_type->name }} @if(!$permission_type->pivot->enabled_by_default)<small class="text-muted">(disabled for new)</small>@endif</label></div></div>
                  @endforeach
                @endforeach
              @endif
            </div>
        ...
        <div class="row check_group">
            ...
            <div class="col-md-9">
                ...
                <div class="col-md-12">
                  ...
                  <div class="col-md-12">
                    <div class="checkbox">
                      <label>{!! Form::checkbox('permissions[]', 'sell.document.share', in_array('sell.document.share', $role_permissions), ['class' => 'input-icheck']); !!} Share invoices, quotations and sales documents using public links</label>
                    </div>
                  </div>
                  <div class="col-md-12">
                    <div class="checkbox">
      ...
      <hr>
        ...
        @endif
        @include('role.partials.restaurant_operations_permissions')
