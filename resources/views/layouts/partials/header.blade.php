...
@php
    ...
    $__accessible_businesses = Auth::user()->accessibleBusinesses()
        ...
        ->get();
    $__active_business = $__accessible_businesses->firstWhere('id', (int) session('user.business_id'));
@endphp
<!-- Main Header -->
    ...
    <div class="tw-px-5 tw-py-3">
        <div class="tw-flex tw-items-start tw-justify-between tw-gap-6 lg:tw-items-center">
            ...
            <div class="tw-flex tw-flex-wrap tw-items-center tw-justify-end tw-gap-3">
                ...
                @if(!empty($__active_business))
                    <details class="tw-dw-dropdown tw-relative tw-inline-block tw-text-left">
                        <summary title="Switch company" aria-label="Current company: {{ $__active_business->name }}. Industry: {{ optional($__active_business->industry)->name ?: 'Industry profile not assigned' }}. Open company switcher."
                            class="tw-inline-flex tw-transition-all tw-ring-1 tw-ring-white/10 tw-cursor-pointer tw-duration-200 theme-btn-bg tw-py-1.5 tw-px-3 tw-rounded-lg tw-items-center tw-justify-center tw-text-sm tw-font-medium tw-text-white hover:tw-text-white tw-gap-2">
                            <i class="fa fa-building" aria-hidden="true"></i>
                            <span class="tw-hidden md:tw-inline-flex tw-items-center tw-gap-1 tw-min-w-0" style="max-width: 20rem">
                                <span class="tw-truncate tw-font-medium">{{ $__active_business->name }}</span>
                                <span class="tw-opacity-60" aria-hidden="true">&mdash;</span>
                                <span class="tw-truncate tw-text-xs tw-opacity-80">{{ optional($__active_business->industry)->name ?: 'Industry profile not assigned' }}</span>
                            </span>
                            <i class="fa fa-angle-down" aria-hidden="true"></i>
                        </summary>
                        <ul class="tw-p-2 tw-w-72 tw-absolute {{ $is_rtl ? 'tw-left-0' : 'tw-right-0' }} tw-z-20 tw-mt-2 tw-origin-top-right tw-bg-white tw-rounded-lg tw-shadow-lg tw-ring-1 tw-ring-gray-200 focus:tw-outline-none"
                            role="menu" tabindex="-1">
                            <li class="tw-px-3 tw-py-2 tw-border-b tw-border-gray-100">
                                <p class="tw-text-xs tw-font-semibold tw-uppercase tw-text-gray-500 tw-m-0">Current company</p>
                                <p class="tw-text-sm tw-font-semibold tw-text-gray-900 tw-mt-1 tw-mb-0">{{ $__active_business->name }}</p>
                                <p class="tw-text-xs tw-text-gray-500 tw-m-0">{{ optional($__active_business->industry)->name ?: 'Industry profile not assigned' }}</p>
                            </li>
                            @foreach($__accessible_businesses as $__company)
                                <li>
                                    @if((int) $__company->id === (int) $__active_business->id)
                                        <span class="tw-flex tw-items-start tw-gap-2 tw-px-3 tw-py-2 tw-text-sm tw-bg-primary-50 tw-rounded-lg" aria-current="true">
                                            <i class="fa fa-check-circle tw-text-primary-600 tw-mt-1" aria-hidden="true"></i>
                                            <span>
                                                <span class="tw-block tw-font-medium tw-text-gray-900">{{ $__company->name }}</span>
                                                <span class="tw-block tw-text-xs tw-text-gray-500">{{ optional($__company->industry)->name ?: 'Legacy company' }}</span>
                                            </span>
                                        </span>
                                    @else
                                        {!! Form::open(['route' => 'business.switch', 'method' => 'post']) !!}
                                            {!! Form::hidden('business_id', $__company->id) !!}
                                            <button type="submit" class="tw-w-full tw-text-left tw-flex tw-items-start tw-gap-2 tw-px-3 tw-py-2 tw-text-sm tw-text-gray-600 tw-transition-all tw-duration-200 tw-rounded-lg hover:tw-text-gray-900 hover:tw-bg-gray-100">
                                                <i class="fa fa-circle-o tw-mt-1" aria-hidden="true"></i>
                                                <span>
                                                    <span class="tw-block tw-font-medium">{{ $__company->name }}</span>
                                                    <span class="tw-block tw-text-xs tw-text-gray-500">{{ optional($__company->industry)->name ?: 'Legacy company' }}</span>
                                                </span>
                                            </button>
                                        {!! Form::close() !!}
                            ...
                            @endforeach
                            @if(Auth::user()->ownedBusinesses()->exists())
                                <li class="tw-border-t tw-border-gray-100 tw-mt-1 tw-pt-1">
                                    <a href="{{ route('business.additional.create') }}"
                                        class="tw-flex tw-items-center tw-gap-2 tw-px-3 tw-py-2 tw-text-sm tw-font-medium tw-text-gray-600 tw-transition-all tw-duration-200 tw-rounded-lg hover:tw-text-gray-900 hover:tw-bg-gray-100">
