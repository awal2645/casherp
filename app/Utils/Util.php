...
class Util
{
    ...
    public function generateToken()
    {
        do {
            $token = Str::random(64);
        } while (Transaction::where('invoice_token', $token)->exists());
    ...
    public function parseNotifications($notifications)
    {
        ...
        foreach ($notifications as $notification) {
            ...
            if (in_array($notification->type, [\App\Notifications\RecurringInvoiceNotification::class, \App\Notifications\RecurringExpenseNotification::class])) {
                ...
                // Native CashERP notifications use a small, stable payload so
                // they remain visible even when no addon registers a parser.
                if (! $parsedByModule && ! empty($data['msg'])) {
                    $notifications_data[] = [
                        'msg' => strip_tags((string) $data['msg']),
                        'icon_class' => $data['icon_class'] ?? 'fas fa-bell bg-blue',
                        'link' => $data['link'] ?? '#',
                        'show_popup' => ! empty($data['show_popup']),
                        'read_at' => $notification->read_at,
                        'created_at' => $notification->created_at->diffForHumans(),
                    ];
                }
