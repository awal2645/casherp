...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            $is_admin = auth()->user()->hasRole('Admin#'.session('user.business_id')) || auth()->user()->can('superadmin');
            $is_business_owner = (int) session('business.owner_id') === (int) auth()->id();
            $procurement_enabled = app(\App\Services\FeatureAccessService::class)->enabled('procurement');
            $smart_documents_enabled = app(\App\Services\FeatureAccessService::class)->enabled('smart_documents');
            $restaurant_operations_enabled = app(\App\Services\FeatureAccessService::class)->enabled('restaurant_operations');
            $company_hub_enabled = app(\App\Services\FeatureAccessService::class)->enabled('company_hub');
            //Home
            ...
            if (app(\App\Services\FeatureAccessService::class)->enabled('property_management') && auth()->user()->can('property.view')) {
                $menu->url(
                    action([\App\Http\Controllers\PropertyManagementController::class, 'index']),
                    'Property Management',
                    [
                        ...
                        'active' => request()->segment(1) == 'property-management',
                    ]
                )->order(7);
            }

            if ($company_hub_enabled && ($is_admin || auth()->user()->can('company_hub.view'))) {
                $menu->dropdown(
                    'Company Hub',
                    function ($sub) use ($is_admin) {
                        $sub->url(action([\App\Http\Controllers\CompanyHubController::class, 'index']), 'Home & announcements', [
                            'icon' => '', 'active' => request()->segment(1) === 'company-hub' && request()->segment(2) === null,
                        ]);
                        $sub->url(action([\App\Http\Controllers\CompanyHubResourceController::class, 'index']), 'Knowledge & documents', [
                            'icon' => '', 'active' => request()->segment(1) === 'company-hub' && request()->segment(2) === 'resources',
                        ]);
                        $sub->url(action([\App\Http\Controllers\CompanyHubEventController::class, 'index']), 'Internal calendar', [
                            'icon' => '', 'active' => request()->segment(1) === 'company-hub' && request()->segment(2) === 'events',
                        ]);
                        $sub->url(action([\App\Http\Controllers\CompanyHubController::class, 'directory']), 'Staff directory', [
                            'icon' => '', 'active' => request()->segment(1) === 'company-hub' && request()->segment(2) === 'directory',
                        ]);
                        if ($is_admin || auth()->user()->can('company_hub.manage_settings')) {
                            $sub->url(action([\App\Http\Controllers\CompanyHubController::class, 'settings']), 'Hub settings', [
                                'icon' => '', 'active' => request()->segment(1) === 'company-hub' && request()->segment(2) === 'settings',
                            ]);
                        }
                    },
                    [
                        'icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M3 21h18"></path><path d="M5 21v-14l8 -4v18"></path><path d="M13 9h6v12"></path><path d="M9 9v.01"></path><path d="M9 12v.01"></path><path d="M9 15v.01"></path><path d="M9 18v.01"></path><path d="M16 13v.01"></path><path d="M16 16v.01"></path></svg>',
                        'active' => request()->segment(1) === 'company-hub',
                    ]
                )->order(8);
            }

            if ($smart_documents_enabled && ($is_admin || auth()->user()->hasAnyPermission(['smart_documents.view', 'smart_documents.create']))) {
                $menu->dropdown(
                    'Smart Documents',
                    function ($sub) use ($is_admin) {
                        if ($is_admin || auth()->user()->can('smart_documents.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\BusinessDocumentController::class, 'index']),
                                'All documents',
                                ['icon' => '', 'active' => request()->segment(1) === 'smart-documents' && request()->segment(2) !== 'create' && request()->segment(2) !== 'settings']
                            );
                        }
                        if ($is_admin || auth()->user()->can('smart_documents.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\BusinessDocumentController::class, 'create']),
                                'Create document',
                                ['icon' => '', 'active' => request()->segment(1) === 'smart-documents' && request()->segment(2) === 'create']
                            );
                        }
                        if ($is_admin || auth()->user()->can('smart_documents.settings')) {
                            $sub->url(
                                action([\App\Http\Controllers\BusinessDocumentController::class, 'settings']),
                                'Document settings',
                                ['icon' => '', 'active' => request()->segment(1) === 'smart-documents' && request()->segment(2) === 'settings']
                            );
                        }
                    },
                    [
                        'icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M14 3v4a1 1 0 0 0 1 1h4"></path><path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z"></path><path d="M9 13h6"></path><path d="M9 17h6"></path></svg>',
                        'active' => request()->segment(1) === 'smart-documents',
                    ]
                )->order(32);
            }

            if ($is_admin || $is_business_owner || auth()->user()->hasAnyPermission(['data_import.view', 'data_import.manage'])) {
                $menu->url(
                    action([\App\Http\Controllers\DataImportController::class, 'index']),
                    'Data Imports',
                    [
                        'icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M12 3v12"></path><path d="M8 11l4 4l4 -4"></path><path d="M4 17v3h16v-3"></path></svg>',
                        'active' => request()->segment(1) === 'data-imports',
                    ]
                )->order(33);
            }
            ...
            //Purchase dropdown
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update') || ($procurement_enabled && (auth()->user()->hasAnyPermission(['procurement.audit.view', 'procurement.quote.manage', 'procurement.approve.manager', 'procurement.approve.finance', 'procurement.approve.admin']) || (auth()->user()->can('procurement.submit') && auth()->user()->hasAnyPermission(['purchase_requisition.create', 'purchase_order.create'])))))) {
                $menu->dropdown(
                    __('purchase.purchases'),
                    function ($sub) use ($common_settings, $procurement_enabled) {
                        if ($procurement_enabled && !empty($common_settings['enable_purchase_requisition']) && (auth()->user()->can('purchase_requisition.view_all') || auth()->user()->can('purchase_requisition.view_own') || (auth()->user()->can('purchase_requisition.create') && auth()->user()->can('procurement.submit')) || auth()->user()->hasAnyPermission(['procurement.audit.view', 'procurement.quote.manage', 'procurement.approve.manager', 'procurement.approve.finance', 'procurement.approve.admin']))) {
                            $sub->url(
                        ...
                        if ($procurement_enabled && !empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own') || (auth()->user()->can('purchase_order.create') && auth()->user()->can('procurement.submit')) || auth()->user()->hasAnyPermission(['procurement.audit.view', 'procurement.approve.manager', 'procurement.approve.finance', 'procurement.approve.admin']))) {
                            $sub->url(
            ...
            // Global order-to-cash workspace. Specialist restaurant, hotel and
            // property operations remain in their own industry modules.
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return', 'so.view_own', 'so.view_all', 'so.create', 'quotation.view_all', 'quotation.view_own', 'draft.view_all', 'draft.view_own'])) {
                $menu->dropdown(
                    'Sales',
                    function ($sub) use ($enabled_modules) {
                        $sub->url(
                            ...
                            'Sales workspace',
                            ['icon' => '', 'active' => in_array(request()->segment(1), ['sales', 'commercial', 'sells', 'sales-order', 'sell-return', 'shipments'], true)]
                        );
                        if (in_array('add_sale', $enabled_modules, true) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'create']),
                                'Create invoice',
                                ['icon' => '', 'active' => request()->segment(1) === 'sells' && request()->segment(2) === 'create' && ! request()->get('status')]
                            );
                        }
                        if (in_array('pos_sale', $enabled_modules, true) && auth()->user()->can('sell.create')) {
                            $sub->url(
                    ...
                    ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M4 19l16 0"></path><path d="M4 15l4 -6l4 2l4 -7l4 3"></path></svg>', 'id' => 'tour_step7']
                )->order(30);
            }
            ...
            // Accounting is grouped by financial task; underlying legacy reports
            // remain available from the React workspace during migration.
            if (auth()->user()->can('account.access') && in_array('account', $enabled_modules, true)) {
                $menu->url(
                    route('accounting.workspace'),
                    'Accounting',
                    [
                        'icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M3 6l18 0"></path><path d="M8 6v12"></path><path d="M16 6v12"></path><path d="M5 18h14"></path><path d="M12 3l9 3h-18z"></path></svg>',
                        'active' => in_array(request()->segment(1), ['accounting', 'account'], true),
                    ]
                )->order(50);
            }
            ...
            if ($restaurant_operations_enabled
                && (auth()->user()->can('restaurant.dashboard.view') || auth()->user()->can('sell.view'))) {
                $menu->url(
                    route('restaurant-operations.dashboard'),
                    'Restaurant Operations',
                    [
                        'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5 tw-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M7 3v8a3 3 0 0 1-3 3V3m3 0v18M17 3v18m0-18c-3 2-3 7 0 9"/></svg>',
                        'active' => request()->segment(1) == 'restaurant-operations',
                    ]
                )->order(64);
            }
            ...
            if (auth()->user()->can('business_settings.access') ||
                ...
                $is_business_owner ||
                auth()->user()->can('premium_modules.purchase') ||
                auth()->user()->can('business.account_closure.manage') ||
                auth()->user()->can('access_package_subscriptions')) {
                $menu->dropdown(
                    __('business.settings'),
                    function ($sub) use ($enabled_modules, $is_business_owner) {
                        if (auth()->user()->can('business_settings.access')) {
                            $sub->url(
                        ...
                        if ($is_business_owner || auth()->user()->can('premium_modules.purchase')) {
                            $sub->url(
                                route('premium-modules.index'),
                                'Premium modules & capacity',
                                ['icon' => '', 'active' => request()->segment(1) == 'premium-modules']
                            );
                        }

                        if ($is_business_owner || auth()->user()->can('business.account_closure.manage')) {
                            $sub->url(
                                route('business.account-closure.show'),
                                'Company account & data',
                                ['icon' => '', 'active' => request()->segment(2) == 'account-closure']
                            );
                        }
