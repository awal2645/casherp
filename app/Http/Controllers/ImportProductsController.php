    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('products_csv')) {
                ...
                $file = $guard->file($request, 'products_csv');

                $parsed_array = Excel::toArray([], $file);

                //Remove header row
                $imported_data = array_splice($parsed_array[0], 1);
                $guard->rows($request, $imported_data, 'products_csv');
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if (! empty($image_name)) {
                        $guard->rejectRemoteReference($image_name, 'Product image', $row_no);
                        $product_array['image'] = basename($image_name);
                    } else {
                        $product_array['image'] = '';
                    }
