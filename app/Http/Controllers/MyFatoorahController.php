...
//-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    public function __construct(SubscriptionPricingService $pricing) {
...
//-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    public function index() {
        abort_unless(auth()->check() && auth()->user()->can('superadmin.access_package_subscriptions'), 403);
        ...
        try {
            ...
            $user_id = (int) auth()->id();
            $package = $this->pricing->packageForBusiness((int) request('package_id'), $business_id, auth()->user()->can('superadmin'));
            $this->pricing->assertCanSubscribe($package, $business_id);
            $quote = $this->pricing->quote($package, $business_id, request('coupon_code'), true);
            $package_id = $package->id;
            $email = auth()->user()->email;
            $amount = $quote['amount'];
            $currency = strtoupper(System::getCurrency()->code);
            $coupon_code = $quote['coupon_code'];
            $name = trim(auth()->user()->user_full_name ?: auth()->user()->username);
            $language = auth()->user()->language ?: config('app.locale');
            $checkoutToken = Str::random(48);

            session(['myfatoorah_checkout' => [
                'token' => $checkoutToken,
            ...
            $curlData = $this->getPayLoadData($package_id, $amount, $currency, $coupon_code, $email, $name, $user_id, $business_id, $language, $checkoutToken);
...
//-----------------------------------------------------------------------------------------------------------------------------------------
     ...
     * @return array
     */
    private function getPayLoadData($package_id, $amount, $currency, $coupon_code, $email, $name, $user_id, $business_id, $language, $checkoutToken) {
        ...
        return [
            ...
            'UserDefinedField'   => json_encode(['business_id' => $business_id, 'coupon_code' => $coupon_code, 'user_id' => $user_id, 'checkout_token' => $checkoutToken]),
