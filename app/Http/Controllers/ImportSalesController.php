...
class ImportSalesController extends Controller
{
    ...
    public function preview(Request $request)
    {
        ...
        if ($request->hasFile('sales')) {
            ...
            $file = $guard->file($request, 'sales');
            $file_name = Str::uuid().'.'.strtolower($file->getClientOriginalExtension());
            $path = $file->storeAs('legacy-imports/sales/'.$business_id.'/'.auth()->id(), $file_name, 'imports_private');
            $parsed_array = $this->__parseData($path);
            $guard->rows($request, array_slice($parsed_array, 1), 'sales');
            $file_token = Str::random(64);
            $request->session()->put('sales_import_files.'.$file_token, [
                'path' => $path,
            ...
            return view('import_sales.preview')->with(compact('parsed_array', 'import_fields', 'file_token', 'business_locations', 'match_array'));
        }
    }

    public function __parseData($storagePath)
    {
        $array = Excel::toArray([], $storagePath, 'imports_private')[0];
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            $fileDetails = $request->session()->pull('sales_import_files.'.$file_token);
            $import_fields = $request->input('import_fields');
            $group_by = $request->input('group_by');
            $location_id = $request->input('location_id');
            $business_id = $request->session()->get('user.business_id');
            if (! is_array($fileDetails)
                || (int) ($fileDetails['business_id'] ?? 0) !== (int) $business_id
                || (int) ($fileDetails['user_id'] ?? 0) !== (int) auth()->id()
                || (int) ($fileDetails['expires_at'] ?? 0) < now()->timestamp) {
                throw ValidationException::withMessages(['sales' => 'The sales import preview has expired or belongs to another company. Upload the file again.']);
            }
            app(\App\Services\DataImport\LegacyImportGuard::class)->location($request, (int) $location_id);

            $file_path = $fileDetails['path'];
            $parsed_array = $this->__parseData($file_path);
            //Remove header row
            ...
            if (! empty($file_path)) {
                Storage::disk('imports_private')->delete($file_path);
            }
