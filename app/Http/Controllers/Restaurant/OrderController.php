...
class OrderController extends Controller
{
    ...
    public function markAsServed($id)
    {
        ...
        try {
            ...
            $updated = $query->update(['res_line_order_status' => 'served']);

            $output = $updated > 0
                ? ['success' => 1, 'msg' => trans('restaurant.order_successfully_marked_served')]
    ...
    public function printLineOrder(Request $request)
    {
        ...
        try {
            ...
            $request->validate(['line_id' => 'required|integer', 'service_staff_id' => 'nullable|integer']);
            ...
            if (empty($line_orders) || empty($line_orders[0])) {
                abort(404);
            }
            $order = $line_orders[0];
    ...
    private function authorizeOrderView(): void
    {
    ...
    private function authorizeOrderManage(): void
    {
