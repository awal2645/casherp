...
class AccountController extends Controller
{
    ...
    public function index()
    {
        ...
        $users = User::forDropdown($business_id, true, true, true, true);

        return view('account.index')
                ->with(compact('not_linked_payments', 'account_types', 'users'));
    }
    ...
    public function show($id)
    {
        ...
        if (request()->ajax()) {
            ...
            if (! empty(request()->input('created_by'))) {
                $accounts->where('account_transactions.created_by', request()->input('created_by'));
            }
