...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // The React dashboard is now the default authenticated home. Keep the
        // previous dashboard available as an explicit rollback/diagnostic view
        // until its remaining module widgets have React equivalents.
        if (! request()->boolean('legacy')) {
            return response()->file(public_path('casherp-workspace.html'));
        }

        $is_admin = $this->businessUtil->is_admin(auth()->user());
        $onboarding = $this->onboardingProgress->forBusiness($business_id, $user);

        if (! auth()->user()->can('dashboard.data')) {
            return view('home.index', compact('onboarding'));
        }
        ...
        return view('home.index', compact('sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings', 'is_admin', 'onboarding'));
    }
    ...
    public function loadMoreNotifications()
    {
        ...
        if (request()->input('page') == 1) {
            $this->notificationQuery()->whereNull('read_at')->update(['read_at' => now()]);
        }
    ...
    {
        $unread_notifications = $this->notificationQuery()
            ->whereNull('read_at')
        ...
        return [
            'total_unread' => $this->notificationQuery()->whereNull('read_at')->count(),
    ...
    /**
     * Notifications follow the active company context. Only explicitly
     * platform-level messages may omit a company identifier.
     */
    private function notificationQuery()
    {
        ...
        return auth()->user()->notifications()->where(function ($query) use ($businessId, $platformTypes) {
            if ($businessId > 0) {
