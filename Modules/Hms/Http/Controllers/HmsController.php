...
class HmsController extends Controller
{
    ...
    public function index()
    {
        ...
        if (! (auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'hms_module'))) {
            ...
            $housekeeping_summary = [
                'dirty' => HmsRoom::leftJoin(
                    ...
                    ->where('hms_room_types.business_id', $business_id)
                    ->where('hms_rooms.housekeeping_status', 'dirty')
                    ->count(),
                'cleaning' => HmsRoom::leftJoin(
                    'hms_room_types',
                    'hms_rooms.hms_room_type_id',
                    '=',
                    'hms_room_types.id'
                    ...
                    ->where('hms_room_types.business_id', $business_id)
                    ->where('hms_rooms.housekeeping_status', 'cleaning')
                    ->count(),
                'awaiting_inspection' => HmsRoom::leftJoin(
                    'hms_room_types',
                    'hms_rooms.hms_room_type_id',
                    '=',
                    'hms_room_types.id'
                    ...
                    ->where('hms_room_types.business_id', $business_id)
                    ->where('hms_rooms.housekeeping_status', 'cleaned')
                    ->count(),
                'overdue' => HmsHousekeepingTask::where('business_id', $business_id)
        ...
        return view('hms::dashboard.index', compact(
            'booking_chart',
            'past_booking_chart',
            'room_count',
            'housekeeping_summary',
            'unbooked_rooms_by_type',
            'guest_count_tonight',
            'arrive_today',
            'leave_today',
            'today_arrivales',
