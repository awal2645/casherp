...
return new class extends Migration
{
    ...
    {
        if (Schema::hasTable('hms_extras') && ! Schema::hasColumn('hms_extras', 'price_per')) {
            Schema::table('hms_extras', function (Blueprint $table) {
                $table->string('price_per')->nullable()->after('price');
            });
        }

        if (Schema::hasTable('hms_extras') && ! Schema::hasColumn('hms_extras', 'financial_classification')) {
            Schema::table('hms_extras', function (Blueprint $table) {
                $column = $table->string('financial_classification', 40)
                    ->default('revenue')
                    ->index();
                if (Schema::hasColumn('hms_extras', 'price_per')) {
                    $column->after('price_per');
                } elseif (Schema::hasColumn('hms_extras', 'price')) {
                    $column->after('price');
                }
            });
