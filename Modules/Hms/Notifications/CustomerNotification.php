...
class CustomerNotification extends Notification
{
    ...
    {
        $settings = $this->notificationInfo;
        if (empty($settings['email_settings']) && ! empty($settings['business_id'])) {
            $business = Business::find($settings['business_id']);
            $settings['email_settings'] = $business ? ($business->email_settings ?? []) : [];
        }
        (new NotificationUtil())->configureEmail($settings);
