...
class EssentialsServiceProvider extends ServiceProvider
{
    ...
    {
        // Register in every environment; execution still requires the host's
        // Laravel scheduler. This supports the standard "production" APP_ENV
        // as well as installations using a custom environment name.
        $this->app->booted(function () {
            $schedule = $this->app->make(Schedule::class);
            $schedule->command('pos:autoClockOutUser')
                ->everyFiveMinutes()
                ->withoutOverlapping();
            $schedule->command('pos:snapshotHrmMetrics')
                ->dailyAt('00:20')
                ->withoutOverlapping();
            $schedule->command('pos:maintainHrmRecords')
                ->dailyAt('00:35')
                ->withoutOverlapping();
        });
    }
    ...
    protected function registerCommands()
    {
        $this->commands([
            ...
            \Modules\Essentials\Console\SnapshotHrmMetrics::class,
            \Modules\Essentials\Console\MaintainHrmRecords::class,
        ]);
    }
}
