...
@section('content')
    ...
    <section class="content">
        ...
        @if(!empty($paymentAttempts) && $paymentAttempts->isNotEmpty())
            @component('components.widget')
                <div class="box-header">
                    <h3 class="box-title">Recent DPO Pay subscription payments</h3>
                </div>
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach($paymentAttempts as $attempt)
                                <tr>
                                    <td>{{ $attempt->reference }}</td>
                                    <td>{{ data_get($attempt->package_snapshot, 'name', 'Package') }}</td>
                                    <td>{{ $attempt->currency_code }} {{ number_format((float) $attempt->amount, 2) }}</td>
                                    <td>
                                        <span class="label label-{{ $attempt->status === 'paid' ? 'success' : ($attempt->status === 'failed' ? 'danger' : 'warning') }}">
                                    ...
                                    <td>
                                        @if($attempt->status === 'pending')
                                            <form method="post" action="{{ route('subscription.dpo.verify', $attempt->id) }}" class="d-inline">
                                                @csrf
                                                <button type="submit" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-primary">
                                                    Check payment status
                                                </button>
                                            </form>
                                        @elseif($attempt->status === 'paid')
