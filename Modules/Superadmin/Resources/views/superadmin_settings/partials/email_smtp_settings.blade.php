    ...
    <div class="row">
        ...
        <div class="col-xs-4">
            <div class="form-group">
                ...
                {!! Form::password('MAIL_PASSWORD', [
                    ...
                    'placeholder' => !empty($default_values['MAIL_PASSWORD_CONFIGURED'])
                        ? 'Saved securely - leave blank to keep it'
                        : __('superadmin::lang.mail_password')
                ]); !!}
                @if(!empty($default_values['MAIL_PASSWORD_CONFIGURED']))
                    <label class="tw-text-xs tw-font-normal tw-mt-2">
                        {!! Form::checkbox('CLEAR_MAIL_PASSWORD', 1, false) !!} Remove the saved system SMTP password
                    </label>
                @endif
            </div>
        </div>
        <div class="col-xs-4">
            <div class="form-group">
                ...
                {!! Form::select('MAIL_ENCRYPTION', ['tls' => 'TLS / STARTTLS', 'ssl' => 'SSL'], $default_values['MAIL_ENCRYPTION'], ['class' => 'form-control']); !!}
            </div>
        </div>
        <div class="clearfix"></div>
        <div class="col-xs-4">
        ...
        <div class="col-xs-12">
            <div class="alert alert-info tw-mt-2">
                <strong>Delivery guidance:</strong> verify the From address with your provider, publish SPF and DKIM records, add a DMARC policy, and use an app password when multi-factor authentication is enabled.
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group">
