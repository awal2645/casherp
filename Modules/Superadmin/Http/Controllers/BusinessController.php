...
use App\Business;
use App\Http\Requests\AdminStoreBusinessRequest;
use App\Industry;
use App\Product;
...
class BusinessController extends BaseController
{
    ...
    protected $featureProvisioning;

    protected $industryOnboarding;
    ...
    public function __construct(
        ...
        IndustryFeatureProvisioningService $featureProvisioning,
        IndustryOnboardingService $industryOnboarding
    )
    {
        $this->businessUtil = $businessUtil;
        $this->moduleUtil = $moduleUtil;
        $this->featureProvisioning = $featureProvisioning;
        $this->industryOnboarding = $industryOnboarding;
    }
    ...
    public function create()
    {
        ...
        $is_admin = true;
        $industryModels = $this->industryOnboarding->selectableIndustries();
        $industries = $industryModels->pluck('name', 'id');
        $industryProfiles = $this->industryOnboarding->profilePayload($industryModels);
     ...
     * @return Response
     */
    public function store(AdminStoreBusinessRequest $request)
    {
        if (! auth()->user()->can('superadmin')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $industry = Industry::whereKey($data['industry_id'])
            ...
            ->firstOrFail();
        $answers = $this->industryOnboarding->sanitizeAnswers(
            $industry,
            $data['onboarding'] ?? []
        );
        ...
        try {
            ...
            //Create owner.
            $owner_details = collect($data)->only(['surname', 'first_name', 'last_name', 'username', 'email', 'password'])->all();
            $owner_details['language'] = config('app.locale');

            $user = User::create_user($owner_details);

            $business_details = collect($data)->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'time_zone', 'accounting_method', 'fy_start_month', 'industry_id'])->all();

            $business_location = collect($data)->only(['name', 'country', 'state', 'city', 'zip_code', 'landmark', 'website', 'mobile', 'alternate_number'])->all();
            ...
            $business_details['enabled_modules'] = $this->featureProvisioning->defaultCoreModules($industry, $answers);
            ...
            $this->featureProvisioning->provision($business, $industry);
            ...
            $subscription_details = collect($data)->only(['package_id', 'paid_via', 'payment_transaction_id'])->all();
        ...
        if (config('app.env') !== 'demo') {
            try {
                ...
                \Log::error('Super Admin created a business but an addon onboarding hook failed.', [
