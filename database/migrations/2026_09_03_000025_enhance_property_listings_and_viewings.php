...
return new class extends Migration
{
    ...
    {
        if (Schema::hasTable('properties') && ! Schema::hasColumn('properties', 'description')) {
        ...
        if (Schema::hasTable('property_units') && ! Schema::hasColumn('property_units', 'listing_purpose')) {
            Schema::table('property_units', function (Blueprint $table) {
                $table->string('listing_purpose', 20)->default('rent');
                $table->decimal('asking_price', 22, 4)->nullable();
                $table->date('available_from')->nullable();
                $table->boolean('is_listed')->default(false);
                $table->string('listing_status', 20)->default('draft');
