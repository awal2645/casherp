...
/**
 * CashERP V21 is an overlay. Core Ultimate POS tables normally come from the
 * installer SQL dump, not from Laravel migrations. This baseline creates those
 * tables only when they are missing so `php artisan migrate` can run on an
 * empty local database without colliding with a real CashERP install.
 */
return new class extends Migration
{
    public function up(): void
    {
        ...
        $this->createIfMissing('packages', function (Blueprint $table) {
            ...
            $table->boolean('is_active')->default(1);
            $table->boolean('mark_package_as_popular')->default(0);
        ...
        $this->createIfMissing('superadmin_coupons', function (Blueprint $table) {
            $table->id();
            $table->string('coupon_code');
            $table->string('discount_type');
            $table->decimal('discount', 22, 4);
            $table->date('expiry_date')->nullable();
            $table->string('applied_on_packages')->nullable();
            $table->string('applied_on_business')->nullable();
            $table->boolean('is_active')->default(1);
            $table->timestamps();
        });
