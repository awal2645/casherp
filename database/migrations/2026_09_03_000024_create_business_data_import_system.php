    ...
    public function up(): void
    {
        ...
        if (Schema::hasTable('packages') && ! Schema::hasColumn('packages', 'data_import_enabled')) {
            Schema::table('packages', function (Blueprint $table) {
                $table->boolean('data_import_enabled')->default(true)->after('invoice_count');
                $table->unsignedInteger('monthly_import_rows')->default(50000)->after('data_import_enabled')
                    ->comment('Zero means unlimited.');
                $table->unsignedInteger('max_import_rows_per_file')->default(10000)->after('monthly_import_rows');
                $table->unsignedSmallInteger('max_import_file_size_mb')->default(10)->after('max_import_rows_per_file');
                $table->unsignedSmallInteger('concurrent_imports')->default(2)->after('max_import_file_size_mb');
                $table->unsignedSmallInteger('import_rollback_days')->default(7)->after('concurrent_imports');
            });
        }
